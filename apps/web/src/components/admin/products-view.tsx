  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        {canManage && (
          <button className="rounded-xl bg-stone-950 px-5 py-3 text-sm font-semibold text-white hover:bg-amber-700" onClick={() => openForm(null)} type="button">
            + Nuevo producto
          </button>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/35" role="dialog" aria-modal="true">
          ...
          <aside className="relative h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8">
            <div className="mb-7 flex items-start justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-700">Catálogo</p>
                <h2 className="mt-1 text-2xl font-semibold">{editing ? "Editar producto" : "Nuevo producto"}</h2>
              </div>
              <button className="grid h-10 w-10 place-items-center rounded-xl bg-stone-100 text-xl" onClick={() => setShowForm(false)} type="button">
                ×
              </button>
            </div>
            <ProductForm
...
function ProductForm({ categories, editing, error, busy, imageFileNames, imagePreviews, onImagesChange, onSubmit }: { categories: Category[]; editing: Product | null; error: string; busy: boolean; imageFileNames: string[]; imagePreviews: string[]; onImagesChange: (files: File[]) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return (
    <form className="space-y-5" key={editing?.id ?? "new"} onSubmit={onSubmit}>
      ...
      <Field defaultValue={editing?.slug} label="Slug (se genera si lo dejás vacío)" name="slug" placeholder="cinturon-toro" required={false} />
      <label className="block text-sm font-medium text-stone-700">
        <span className="mb-1.5 block">Categoría</span>
        <select className="control" defaultValue={editing?.categoryId ?? ""} name="categoryId">
      ...
      <label className="block text-sm font-medium text-stone-700">
        ...
        <textarea className="control min-h-24 resize-y" defaultValue={editing?.description ?? ""} name="description" placeholder="Detalles del producto" />
      </label>
      <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field defaultValue={editing?.brand ?? ""} label="Marca" name="brand" placeholder="InfinityShop" required={false} />
        <Field defaultValue={editing?.tags.join(", ") ?? ""} label="Etiquetas" name="tags" placeholder="dorado, minimalista" required={false} />
      </div>
      <label className="block text-sm font-medium text-stone-700">
        <span className="mb-1.5 block">
          Imágenes <span className="font-normal text-stone-400">(una URL por línea)</span>
        </span>
        <textarea className="control min-h-28 resize-y" defaultValue={editing?.images.join("\n")} name="images" placeholder="https://.../producto.jpg" />
      </label>
      <div>
        <span className="mb-1.5 block text-sm font-medium text-stone-700">Subir imágenes</span>
        <FilePicker accept="image/jpeg,image/png,image/webp,image/avif" buttonLabel="Elegir imágenes" description="Hasta 8 archivos, máximo 5 MB cada uno" id={`product-images-${editing?.id ?? "new"}`} multiple name="imageFiles" onChange={onImagesChange} selectedNames={imageFileNames} />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field defaultValue={String(editing?.featuredOrder ?? 0)} label="Orden destacado" min="0" name="featuredOrder" placeholder="0" type="number" />
        <label className="flex items-center justify-between rounded-xl border border-stone-200 px-4 py-3">
      ...
      <label className="flex items-center justify-between rounded-xl border border-stone-200 px-4 py-3">
        <span>
          ...
          <span className="text-xs text-stone-400">Visible en la tienda pública</span>
        </span>
        <input className="h-5 w-5 accent-amber-700" defaultChecked={editing?.active ?? true} name="active" type="checkbox" />
      </label>
      {error && <p className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}
      <button className="w-full rounded-xl bg-stone-950 px-5 py-3.5 text-sm font-bold text-white hover:bg-amber-700 disabled:opacity-50" disabled={busy} type="submit">
...
function Field({ label, name, placeholder, defaultValue, type = "text", min, step, required = true }: { label: string; name: string; placeholder: string; defaultValue?: string; type?: string; min?: string; step?: string; required?: boolean }) {
  return (
    <label className="block text-sm font-medium text-stone-700">
      <span className="mb-1.5 block">{label}</span>
      <input className="control" defaultValue={defaultValue} min={min} name={name} placeholder={placeholder} required={required} step={step} type={type} />
    </label>
  );
}
