...
async function sendAccountEmail(input: { to: string; subject: string; heading: string; message: string; actionLabel: string; actionUrl: string }): Promise<void> {
...
export function sendEmailVerification(input: { email: string; firstName: string; token: string }): Promise<void> {
  ...
  return sendAccountEmail({
    ...
    message: "Confirmá que esta dirección te pertenece. El enlace vence en 24 horas y solo puede usarse una vez.",
    actionLabel: "Verificar email",
    actionUrl: url.toString(),
  });
}

export function sendPasswordResetEmail(input: { email: string; firstName: string; token: string }): Promise<void> {
...
export function sendTeamInvitationEmail(input: { email: string; tenantName: string; inviterName: string; role: string; token: string }): Promise<void> {
